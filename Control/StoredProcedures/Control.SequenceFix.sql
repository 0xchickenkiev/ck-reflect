...
CREATE PROCEDURE [Control].[SequenceFix]
(	
...
AS
	...
	CREATE TABLE #Issue 	(	Other__$From datetime2(7),
								__$From datetime2(7),
								__$To datetime2(7),
								__$Load datetime2(7),
...
BEGIN
	...
	-- Get a list of Schemas, Tables and their index keys to build the dml
	SET @SQL = 'SELECT	s.Name SchemaName, t.name TableName, c.name PartitionColumn
				...
				SELECT				p.SchemaName, p.TableName, 
									''SELECT Other__$From, __$From, __$To, __$Load, __$Action INTO #x FROM ( SELECT LEAD(__$From) OVER (PARTITION BY '' + 
									...
									''  ORDER BY __$Load) Other__$From, * from Lake_Cerillion.'' + SchemaName + ''.'' + TableName + '') x where Other__$From <= __$From
									UNION ALL
									SELECT Other__$From, __$From, __$To, __$Load, __$Action FROM ( SELECT LAG(__$From) OVER (PARTITION BY '' + 
									STUFF( (SELECT '', '' + PartitionColumn FROM #k as c WHERE c.SchemaName = p.SchemaName AND c.TableName = p.TableName ORDER BY SchemaName, TableName FOR XML PATH('''') ), 1, 1, '''') +
									''  ORDER BY __$Load) Other__$From, * from Lake_Cerillion.'' + SchemaName + ''.'' + TableName + '') x where Other__$From >= __$From
									
									UPDATE a SET a.__$From = b.__$From, a.__$To = b.__$To
									FROM (SELECT Other__$From, __$From, __$To FROM  #x ) b
									INNER JOIN Lake_Cerillion.'' + SchemaName + ''.'' + TableName + '' a ON a.__$From = b.Other__$From '' as dml
							FROM #k	as p
	...
	INSERT @Sequence (SchemaName, TableName, dml) EXECUTE (@SQL)
	SET @i = @@ROWCOUNT

	IF @DryRun = 1 SELECT * FROM @Sequence

	-- For each Table, run the dml statement and return a count of out of order records into @IssueCount
	SET @Message = @Message + CHAR(13) + CHAR(10) + CONVERT(varchar,SYSDATETIME()) + ' - Fixing Sequencing:'
	...
	BEGIN
		...
		SET	@ErrorMessage = NULL
			BEGIN
				...
				SELECT @SQL = dml FROM @Sequence WHERE i = @i
				BEGIN TRY
		...
		SELECT @Message = @Message + CHAR(13) + CHAR(10) + CHAR(9) 
						...
						+ 'Out Of Sequence Records Fixed: ' + OutOfSequence + CHAR(9) 
